...
def addOrderItem():
    if request.method == "POST":
        ...
        if db.execute(f"SELECT * FROM `OrderItem` WHERE `orderId` = '{orderId}' AND `itemId` = '{itemId}'"):
            ...
            db.execute(f"UPDATE `OrderItem` SET `quantity` = `quantity` + {quantity} WHERE `orderId` = '{orderId}' AND `itemId` = '{itemId}';")
    ...
    if orderId:
        ...
        itemData = db.execute(f"SELECT * FROM `Item` WHERE `itemId` = '{itemId}'")
        item = Item(*itemData[0])
        return render_template("addOrderItem.html", order=None, item=item, customerId=None)
...
def showOrders():
    ...
    # Order management - Show all orders with a specific item
        ...
        data = db.execute(f"select * from `Order` where orderId in (SELECT orderId FROM `OrderItem` WHERE `itemId` = '{itemId}')")
        orders = [Order(*row) for row in data]
...
def addOrder():
    ...
    return render_template("addOrder.html", customer="")
...
def cancelOrder():
    ...
    # db.execute(f"DELETE FROM `OrderItem` WHERE `orderId` = '{orderId}';")
    # db.execute(f"DELETE FROM `Order` WHERE `orderId` = '{orderId}';")
